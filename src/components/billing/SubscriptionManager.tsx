...
export default function SubscriptionManager({ userId, onSubscriptionChange }: SubscriptionManagerProps) {
  ...
  const handleSubscribe = async (planId: string) => {
    try {
      ...
      // Get user email from somewhere (you might need to pass it as a prop or get it from auth)
      const userEmail = 'user@example.com'; // Replace with actual user email
      
      const result = await billingService.createSubscription(userId, planId, couponCode || undefined, userEmail);
      ...
      if (result.paymentOrder) {
        ...
        const options = {
          key: import.meta.env.VITE_RAZORPAY_KEY_ID || 'rzp_test_example_key',
          ...
          handler: async (response: any) => {
            try {
              ...
              if (verified) {
                ...
                await paymentService.activatePremiumForUser(userId, planId);
