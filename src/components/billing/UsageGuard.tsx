...
export default function UsageGuard({ 
  ...
  const checkUsage = async () => {
    try {
      ...
      // CRITICAL FIX: Shorter timeout and immediate fallback for development
      const timeout = setTimeout(() => {
        console.warn('UsageGuard timeout - allowing access by default');
      ...
      try {
        // QUICK FALLBACK: If no userId, allow immediately
        if (!userId) {
        ...
        // Load usage stats for display (non-blocking)
        usageEnforcementService.getUserUsageStatus(userId).then(stats => {
          setUsageStats(stats);
        }).catch(err => {
        ...
        clearTimeout(timeout);
        console.error('API Error in UsageGuard:', apiError);
        // Allow access on API error - fail open instead of fail closed
        setCheckResult({ allowed: true });
        setIsBlocked(false);
      }
  ...
  if (loading) {
    // CRITICAL FIX: Show children immediately while loading to prevent UI blocking
    return <>{children}</>;
  }
